...
public class Battle {
    ...
    public void battle(Fighter one, Fighter two) {
        ...
        System.out.println("\nThe Battle is between\n"+ C_BLUE + one.getName() + " with attack " + one.getAttack() + " and " +
                one.getDefence() + " defence\n"+ C_RESET + C_YELLOW +  two.getName() + " with attack " + two.getAttack() + " and " +
                two.getDefence() + " defence\n" + C_RESET);
        scanner.nextLine();
        ...
        while (two.getHealth() > 0 && one.getHealth() > 0) {
            ...
            if (res > 0) {
                ...
                two.loseHealth(res);
                System.out.println(C_YELLOW + two.getName() + " lost " + res + " Hp, " + two.getHealth() + "Hp Left" + C_RESET);
            } else if (res == 0) {
                System.out.println(C_YELLOW + two.getName() + " Blocks the Attack!" + C_RESET);
            } else {
                cou = Math.abs(res);
                System.out.println(C_YELLOW + two.getName() + " Counter attacks with " + cou + C_RESET);
                one.loseHealth(cou);
            ...
            System.out.println(C_YELLOW + two.getName() + " Attacks with " + (dmg = random.nextInt(two.getAttack()))+ C_RESET);
            ...
            if (res > 0) {
                ...
                two.loseHealth(cou);
                System.out.println(C_YELLOW + two.getName() + " lost " + cou + " Hp, " + two.getHealth() + " Hp Left"+C_RESET);
            }
